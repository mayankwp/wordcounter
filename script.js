...
class WordCounter {
    ...
    initializeAutoResize() {
        ...
        // Set initial properties to ensure no scrollbars
        this.textInput.style.height = this.initialHeight + 'px';
        this.textInput.style.overflow = 'hidden';
        this.textInput.style.resize = 'none';
        this.textInput.style.boxSizing = 'border-box';
        
        // Bind auto-resize to multiple events for comprehensive coverage
        const resizeEvents = ['input', 'keyup', 'keydown', 'paste', 'cut', 'delete', 'mouseup'];
        
        resizeEvents.forEach(event => {
            this.textInput.addEventListener(event, () => {
                // Use setTimeout to ensure content is updated before measuring
        ...
        // Backup: Use MutationObserver to catch any missed content changes
    ...
    autoResize() {
        // Force overflow hidden to prevent scrollbars during measurement
        this.textInput.style.overflow = 'hidden';
        
        // Reset height to auto to get true content height
        this.textInput.style.height = 'auto';
        
        // Get the scroll height which represents the full content height
        const scrollHeight = this.textInput.scrollHeight;
        
        // Calculate new height, ensuring it's at least the initial height
        const newHeight = Math.max(this.initialHeight, scrollHeight + 2);
        
        // Set the new height to fit all content
