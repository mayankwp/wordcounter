...
class WordCounter {
    constructor() {
        ...
        this.statisticsConfig = {
            ...
            charactersNoSpaces: { label: 'Characters (no spaces)', default: false, hasTooltip: false },
            ...
            wordsPublisher: { 
                ...
                default: false, 
                hasTooltip: true, 
                tooltip: 'Based on the article: https://wordcounter.net/blog/2016/02/04/101200_publisher-word-count.html',
                link: 'https://wordcounter.net/blog/2016/02/04/101200_publisher-word-count.html'
            },
            readingLevel: { 
                label: 'Reading Level', 
                default: true, 
                hasTooltip: true, 
                tooltip: 'Based on the article: https://wordcounter.net/blog/2015/11/05/10805_writing-reading-level-tool.html',
                link: 'https://wordcounter.net/blog/2015/11/05/10805_writing-reading-level-tool.html'
            },
            readingTime: { 
                label: 'Reading Time', 
                default: true, 
                hasTooltip: true, 
                tooltip: 'Based on an average reading speed of 275 words per minute',
                link: '#'
            },
            speakingTime: { 
                label: 'Speaking Time', 
                default: true, 
                hasTooltip: true, 
                tooltip: 'Based on an average speaking speed of 180 words per minute',
                link: '#'
            },
            writingTime: { 
                label: 'Writing Time', 
                default: false, 
                hasTooltip: true, 
                tooltip: 'Based on an average handwriting speed of 68 letters per minute',
                link: '#'
            }
        };
    ...
    updateAllStats() {
        ...
        this.charCountEl.textContent = text.length.toLocaleString();
            ...
            if (this.enabledStats[key]) {
                ...
                const labelHtml = config.hasTooltip ? 
                    `<span class="stat-label-with-tooltip">
                        ${config.label}
                        <span class="tooltip-icon" data-tooltip="${config.tooltip}" data-link="${config.link}">?</span>
                    </span>` :
    ...
    calculateAllStatistics(text) {
        ...
        stats.readingTime = this.formatTime(Math.ceil(stats.words / 275));
        stats.speakingTime = this.formatTime(Math.ceil(stats.words / 180));
        stats.writingTime = this.formatTime(Math.ceil(charactersNoSpaces / 68));
