...
### July 27, 2025 - Default Statistics Update
- **Characters (no spaces) Default**: Enabled "Characters (no spaces)" statistic by default for new users (changed from false to true)
- **Improved User Experience**: New users will now have both characters with and without spaces displayed by default for comprehensive text analysis

### July 27, 2025 - Copy/Clear Buttons & Mobile Optimization
- **Copy/Clear Functionality**: Added Copy and Clear buttons to the right side of the auto-save section in the input footer
- **Copy Features**: Implemented clipboard API with visual feedback ("Copied!" confirmation) and fallback support for older browsers
- **Clear Functionality**: Added clear button with confirmation dialog for longer text content (100+ characters)
- **Mobile Hamburger Menu**: Added responsive hamburger menu for mobile navigation with smooth animations
- **Mobile-First Responsive Design**: Implemented comprehensive mobile optimization with breakpoints for 768px, 480px, and 360px screens
- **Mobile Layout Adjustments**: Reorganized tool layout for mobile with sidebar below text input, centered controls, and touch-friendly buttons
- **Mobile Text Input**: Set minimum font size to 16px to prevent iOS zoom, optimized textarea height for different screen sizes
- **Mobile Modal**: Enhanced modal design for mobile with proper scrolling, full-width tabs, and touch-friendly interactions
- **Responsive Navigation**: Mobile menu slides down from top with glass morphism effects and auto-closes on link clicks or outside clicks
- **Touch Optimization**: Improved button sizes, spacing, and touch targets for better mobile usability
- **Screen Size Adaptations**: Progressive layout changes for very small screens (360px and below) with optimized spacing and typography

### July 27, 2025 - Full-Screen Mobile Menu Enhancement
- **Full-Screen Mobile Menu**: Redesigned mobile menu to cover entire viewport (100vw x 100vh) starting from top of screen
- **Mobile Menu Header**: Added dedicated header section with logo, title, and close button matching desktop navigation design
- **Enhanced Blur Effects**: Implemented comprehensive backdrop blur effects (24px blur + 180% saturation) for mobile menu background
- **Improved Menu Structure**: Separated mobile menu into header and content sections for better organization and visual hierarchy
- **Mobile Overlay Enhancement**: Updated overlay to cover full screen with enhanced blur effects (8px blur + 150% saturation) for background content
- **Navigation Flow**: Added close button in menu header and improved navigation link interactions with active state styling
- **Z-Index Optimization**: Properly layered mobile menu (z-index: 999) and overlay (z-index: 998) for correct stacking order
- **Mobile UX Improvements**: Enhanced touch interactions, scroll prevention, and smooth transitions for professional mobile experience